...
;(function(window, exportName) {
    ...
    function ExpressRangeValidator(rangeExpress, value, options) {
        ...
        /** R_CODE_SUCCESS:1  R_CODE_FAIL:1  R_CODE_INVALID:2 */
        var R_CODE_SUCCESS = 0, R_CODE_FAIL = 1,  R_CODE_INVALID = 2;
        var runResultCode = R_CODE_SUCCESS;
                ...
                return [
                    { // gMatchResult: ['x>90','x','>','90',undefined,index:0,input:'x>90', groups:undefined]
                        ...
                        run: function () {
                            ...
                            runResultCode = rResult? R_CODE_SUCCESS: R_CODE_FAIL;
                            return rResult;
                        }
                    },
                    ...
                    { // gMatchResult: 
                        pattern: 'x=N,x=M,x=P,x=Q',
                        regex: new RegExp('^([x|X]=' + numRegex + ')(\,([x|X]=' + numRegex + ')){2,}$'),
                        run: function () {
                            let expStr = gMatchResult[0];
                            let reg2 = new RegExp('[x|X]=' + numRegex, 'g');
                            let hasNumber = null;
                            let numberList = [];
                            do {
                                hasNumber = reg2.exec(expStr);
                                hasNumber && numberList.push(hasNumber[1] * 1);
                            } while (hasNumber);
                            var rResult = false;
                            if (numberList.includes(xVariable * 1)) {
                                rResult = true;
                            }
                            runResultCode = rResult ? R_CODE_SUCCESS : R_CODE_FAIL;
                            return rResult;
                    ...
                    { // gMatchResult: 
                        pattern: '(x#N,x#M),(x#P,x#Q),(x#R)',
                        regex: (function () {
                            var unitExp = '\\((.+?)\\)';
                            var reg = new RegExp('^(' + unitExp + ')(,' + unitExp + '){1,}$', 'g');
                            return reg;
                        })(),
                        run: function () {
                            let expStr = gMatchResult[0];
                            let wrapReg = new RegExp('\\((.+?)\\)', 'g');
                            let hasMatchResult = null;
                            let innerRangeExpressList = [];
                            do {
                                hasMatchResult = wrapReg.exec(expStr);
                                hasMatchResult && innerRangeExpressList.push(hasMatchResult[1]);
                            } while (hasMatchResult)
                            let rResult = false;
                            for(var i = 0, len = innerRangeExpressList.length; i < len; i++ ) {
                                var itemExp = innerRangeExpressList[i];
                                var validator = new ExpressRangeValidator(itemExp, xVariable);
                                if (R_CODE_INVALID == validator.runResultCode) {
                                    runResultCode = validator.runResultCode;
                                    return false;
                                }
                                rResult = rResult || validator.runResult;
                            }
